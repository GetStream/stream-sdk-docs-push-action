...
for sdk in "${SDKs[@]}"
do
    find $CLONE_DIR/$PRODUCT_DIRECTORY -maxdepth 1 -mindepth 1 -type f -iname "*${sdk/ /}*" -exec bash $ACTION_PATH/find_match_handler.sh ${sdk/ /} {} \;
    find $CLONE_DIR/$PRODUCT_DIRECTORY -maxdepth 1 -mindepth 1 -type d -iname "*${sdk/ /}*" -exec bash $ACTION_PATH/find_match_handler.sh ${sdk/ /} {} \;
    find $CLONE_DIR/$PRODUCT_DIRECTORY -maxdepth 2 -mindepth 2 -type d -iname "*${sdk/ /}*" -exec bash $ACTION_PATH/find_match_handler.sh ${sdk/ /} {} \;
    ls -la $CLONE_DIR/$PRODUCT_DIRECTORY
    ls -la $CLONE_DIR/$PRODUCT_DIRECTORY/docs
done

# Create Product directory if it doesn't exist
[ -d "$CLONE_DIR/$PRODUCT_DIRECTORY" ] || mkdir "$CLONE_DIR/$PRODUCT_DIRECTORY"

# Copy the documentation from the SDK to the temporary directory
cp -a "./$SOURCE_DIRECTORY/." "$CLONE_DIR/$PRODUCT_DIRECTORY"
